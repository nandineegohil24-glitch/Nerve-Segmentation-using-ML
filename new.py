...
class UltrasoundDataset(Dataset):
    ...
    def __getitem__(self, idx):
        img = self.images[idx][0]  # shape (128,128)
        mask = self.masks[idx]

        # convert to HxWxC for Albumentations
        img = (img * 255.0).astype(np.uint8)
        mask = (mask * 255.0).astype(np.uint8)

        img = np.expand_dims(img, -1)  # (128,128,1)
...
train_dataset = UltrasoundDataset(X_train, y_train, augment=train_transform)
val_dataset   = UltrasoundDataset(X_val, y_val, augment=None)
...
def dice_coeff(pred, target, smooth=1e-6):
    ...
    return torch.clamp(dice, 0, 1).item()
...
# model_epoch10 = load_model("saved_models/unet_epoch10_20251001_xxxxx.pth")
